...
**** Scalar
     ...
     *gcc* don't use vector instruction(*pd*). It use scalar instruction(*sd*).
...
**** Vector
    ...
    *gcc* uses *sd* instruction, and also for others vectors functions
    so I will not show them.

*** clang
**** Scalar

    Same as *gcc*, uses *sd* operations.
...
***** 4 double
      ...
      Here *clang* vectorized too, but not all. We have 4 *movsd* by
      turn of loop. And it vectorize with register vector of 2 double,
      128 bits size. Therefore it vectorized to about half. It does
      the same number of *operation* by turn of loop, that is 4
      multiplication and 4 addition.
