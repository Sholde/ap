...
** Introduction
  ...
  example that is we add 4 at the register because behind it access at
  bytes.
...
**** Scalar
     ...
     *11c7*, *gcc* doesn't use vector instruction (because it is *sd*
     ...
     registers to have vector instructions. And in the instuction
     *add* at line *11c0*, we can see that wa add 1 at the register
     *rax*.
...
***** 2 double
      ...
      *multiplications* and 2 *additions* and 1 *add* of the value 2
      at line *1207*. It didn't vectorize.
...
***** 4 double
      ...
      *gcc* uses *sd* instructions, we add 1 at the iterator register
...
***** 8 double
      ...
      *gcc* uses *sd* instructions, and we add 1 at the iterator
...
**** Scalar
     ...
     *additions* by turn of loop(begin at *14c0* and finish at *14d5*)
     like the *c code*, bacause it add 1 at the register *rax*.
...
**** Vector
    ...
    *gcc* uses *sd* instructions and make as many operations than *c
    code*, because it add 2 at the register *rax*. It is also the same
...
**** Vector
     ...
     *gcc* also make the same for vector functions.
     ...
     bytes* at the register *rax*. It is the same for others vector
     functions.
...
***** 2 double
     ...
     *clang* vectorized the loop(*pd* instructions) and do exactly the
     same numbers of operations than *c code*. It add 2 at the
...
***** 4 double
     ...
     *move* instructions. And it respect the number of operations by
     turn of loop of *c code*. It add 4 at the iterator register
     *rax*.
...
***** 2 double
     ...
     *1312 with the add of 2 at the register *rax*).
